# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- #
# Con/IO' auxilizary encdongs db
# Converted in 2019 by V.A. Guevara
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- #
...
proc nameToCodePage*(name: string): int =
    var nameAsInt: int
    if parseInt(name, nameAsInt) == 0: nameAsInt = -1
    for no, na in items(winEncodings):
        if no == nameAsInt or eqEncodingNames(na, name): return no
    return -1

proc codePageToName*(c: int): string =
    for no, na in items(winEncodings):
        if no == c:
